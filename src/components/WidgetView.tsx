...
export function WidgetView() {
  ...
  return (
    <div style={{ fontFamily: 'Consolas, monospace', background: thm.bg0, color: thm.text1, height: '100vh', display: 'flex', flexDirection: 'column', fontSize: 12 }}>
      {/* Stats bar */}
        ...
        return (
          <div style={{
            ...
            <svg width={22} height={22} style={{ flexShrink: 0 }}>
              <circle cx={11} cy={11} r={9} fill="none" stroke="#1f1f1f" strokeWidth={2.5} />
            ...
            <div style={{ flex: 1, overflow: 'hidden' }}>
              <div style={{ fontSize: 9, color: '#444', marginBottom: 1 }}>{isWork ? 'FOCUS' : 'BREAK'} · {'🍅'.repeat(Math.min(pomodoro.sessionsCompleted, 5))}</div>
            ...
            {isWork ? (
              <button onClick={pausePomodoro} title="Pause" style={miniBtn('#1a1a1a')}>⏸</button>
            ) : (
              <button onClick={() => startPomodoro(pomodoro.taskId!)} title="Skip break" style={miniBtn('#1a1a1a')}>▶</button>
            )}
...
function miniBtn(bg: string): React.CSSProperties {
  return {
    width: 22, height: 22, borderRadius: '50%', border: '1px solid #222',
    background: bg, color: '#888', cursor: 'pointer', fontSize: 9, padding: 0,
